...
###############################choose training or resting#######################
# training = True ###training
training = not True ###testing
...
if training:
    ...
    #############################################
    # Create a folders
    dir_name = 'runs/' + 'DDQN_PER_LSTM_0308' \
               + '_seed_' + str(seed) \
               + '_rho_' + str(rho_c)
    ...
    copy('RLeMPC_PER_LSTM_D.py', dir_name)
    ...
    for frame_idx in range(1, num_frames + 1):
        ...
        if done:
            ...
            if episode % eval_per_train == 0:
                ...
                # save the model
                torch.save(current_model.state_dict(), dir_name + '/save_model/w_' + str(episode) + '.pt')
    ...
    #            + '_seed_' + str(seed) \
    #            + '_rho_' + str(rho_c)
    dir_name = 'runs/' + 'DDQN_PER_LSTM_0306' \
               + '_betaF_' + str(beta_frames) \
               + '_beta0_' + str(beta_start) \
               + '_rho_' + str(rho_c) \
 \
    # path for the model
    PATH = dir_name + '/save_model/' + 'w_410.pt'

    if not os.path.exists(dir_name + "/eval_res"):
        os.mkdir(dir_name + "/eval_res")

    current_model.load_state_dict(torch.load(PATH))
    current_model.eval()
    ...
    while not done_test:
        hidden_state_test, cell_state_test = current_model.init_hidden_states(bsize=1)
        q_value_test, (hidden_state_test, cell_state_test) = current_model.forward(
            torch.from_numpy(state_test).float(), 1,
            hidden_state_test, cell_state_test)
        action_test = q_value_test.max(1)[1].data[0]
        action_test = action_test.item()
        state_test, reward_test, done_test, (t, jmpc) = env_test.step(action_test)
    ...
    scipy.io.savemat(dir_name + '/eval_res/res.mat', savemat)

    print("eval_episode_return:", eval_episode_return)
    print("seed:", seed)